...
describe("utils", () => {
    ...
    test("isUrlAllowed returns false for bad pattern", () => {
        // This pattern creates an invalid regex, so buildSiteRegex returns null
        expect(utils.isUrlAllowed("https://x.com", ["["])).toBe(false);
    });

    test("isUrlAllowedCompiled handles invalid URL", () => {
        const compiled = utils.compileSitePatterns(["*example*"]);
        // URL constructor will throw, but it should be caught and still return false
